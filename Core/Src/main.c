...
double FutureAngle(double acceleration, double time) {
  //this is calculated using Euler's Method, very inaccurate, may need to adjust
  double angle = 0.5 * acceleration * time * time;

  return angle;
}

void DisplayFuture(double angle) {
  int light = 50/3.1415926*angle - 3.1415926/2;
  Set_LED(light, 255, 0, 0);
  WS2812_Send();
}
/* USER CODE END 0 */
